...
async function initAuth(){
  ...
  if(isAdmin){
    ['nav-usuarios','nav-config','nav-sync','nav-rrhh','nav-informes','btn-exportar-clientes'].forEach(id=>{
...
async function loadConectados(){
  ...
  cont.innerHTML = `<table class="tbl"><thead><tr>
    ...
    data.usuarios.map(u=>`<tr>
      <td><span style="display:inline-block;width:9px;height:9px;border-radius:50%;background:${dot[u.estado]}" title="${lbl[u.estado]}"></span></td>
      <td><b>${u.nombre||u.username}</b><br><small style="color:var(--txt2)">${u.username}</small></td>
      <td><span class="badge">${u.rol||'—'}</span></td>
      <td>${u.estado==='en_linea'?'<b style="color:#2e7d32">ahora</b>':'hace '+hace(u.segundos_inactivo)}</td>
      <td><small>${u.ip||'—'}</small></td>
      <td><small style="color:var(--txt2)">${(u.ultima_ruta||'').replace('/api/','')}</small></td>
    </tr>`).join('') + '</tbody></table>';
}
...
const _SECCIONES_CATALOGO = {
  ...
  torres:'Torres', 'torres-enlaces':'Enlaces de Torres', aps:'APs',
  ...
  abonos:'Abonos', bajas:'Bajas', ftth:'FTTH', ftth_plan:'FTTH Plan',
...
async function loadUsoSistema(){
  ...
  const filaSec = (s)=>{
    ...
    return `<tr style="border-bottom:1px solid #eee;${estilo}">
  ...
  const filaUsr = (u)=>`<tr style="border-bottom:1px solid #eee">
      <td style="padding:.3rem .5rem"><b>${u.username}</b></td>
      ...
      <td style="padding:.3rem .5rem;font-size:.8rem">${u.ultima_vez||'—'}</td>
    </tr>`;
  ...
  cont.innerHTML = `
    ...
    ${nunca.length ? `<div style="margin-top:.6rem;padding:.5rem .7rem;background:#fff8e1;border:1px solid #ffe082;border-radius:6px;font-size:.83rem">

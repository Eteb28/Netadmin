...
async function loadNapDatalist(){
  ...
  if(!naps) return;
  napData=naps;
  document.getElementById('nap-datalist').innerHTML=naps.map(n=>`<option value="${escAttr(n.nombre)}">`).join('');
}
...
function drawTopNaps(naps) {
  ...
  wrap.innerHTML = naps.slice(0, 5).map((n, i) => {
    ...
    return `<div class="top-nap-row" onclick="showNapDetail('${escJs(n.nombre)}')">
      <div class="top-nap-rank ${rankClass}">${i+1}</div>
      <div class="top-nap-info">
        <div class="top-nap-name">${escHtml(n.nombre)}</div>
        <div class="top-nap-stats">${n.total}/${n.capacidad} puertos · ${escHtml(n.localidad)||'—'}${n.libre <= 0 ? ' · 🔴 LLENO': ''}</div>
      </div>
...
function drawNapsAgrupadas(grupos) {
  ...
  wrap.innerHTML = grupos.slice(0, 15).map((g, i) => {
    ...
    const napsHtml = g.naps.slice(0, 12).map(n => {
      ...
      return `<div class="nap-mini ${cls}" onclick="event.stopPropagation();showNapDetail('${escJs(n.nombre)}')">
        <div class="nap-mini-name" title="${escAttr(n.nombre)}">${escHtml(n.nombre.split(' - ').slice(1).join(' ').replace(/CDO:/g,'CDO').replace(/NAP:/g,'NAP')||n.nombre)}</div>
    ...
    return `<div class="nap-group ${open}">
      <div class="nap-group-header" onclick="this.parentElement.classList.toggle('open')">
        ...
        <span class="nap-group-name">📍 ${escHtml(g.nombre)}</span>
...
async function showNapDetail(nombre){
  ...
  const cliRows=n.clientes.map(c=>`<tr>
    <td>${c.nro_cliente?`<span style="font-family:monospace;color:#1565c0;font-weight:600">#${escHtml(c.nro_cliente)}</span> `:''}${escHtml(c.nombre)}</td>
  ...
  const censoInfo = n.censada
    ? `<div style="background:var(--card);border-radius:8px;padding:.5rem .7rem;font-size:.8rem;margin-top:.6rem">
         ✅ <b>Censada</b> el ${escHtml(n.censo_fecha)||'—'} por <b>${escHtml(n.censo_tecnico)||'—'}</b>${n.censo_asistente?` (asistió: ${escHtml(n.censo_asistente)})`:''}
         ${n.censo_bocas_fisicas!=null?`<br>Bocas físicas contadas: <b>${n.censo_bocas_fisicas}</b> · registrados en sistema: ${n.total}${n.censo_bocas_fisicas!=n.total?' <span style="color:#c62828">⚠ discrepancia</span>':' ✓'}`:''}
         ${n.censo_notas?`<br><small>${escHtml(n.censo_notas)}</small>`:''}
         <button class="btn btn-gray btn-xs" style="margin-left:.5rem" onclick="abrirCensoNap(${n.id},'${escJs(n.nombre)}',${n.total})">Recensar</button>
       </div>`
    : `<div style="background:var(--tint-ambar);border-radius:8px;padding:.5rem .7rem;font-size:.8rem;margin-top:.6rem">
         ⚠ <b>Sin censar.</b> <button class="btn btn-am btn-xs" onclick="abrirCensoNap(${n.id},'${escJs(n.nombre)}',${n.total})">📋 Censar ahora</button>
       </div>`;
...
async function reloadMapaNaps(){
  ...
  data.forEach(n=>{
    ...
    nc.slice(0,30).forEach(c=>{
      ...
      dotsHtml+=`<span style="background:${dc};width:10px;height:10px;border-radius:50%;display:inline-block;margin:1px" title="${escHtml(c.nombre)} (${escHtml(c.estado)})"></span>`;
    });
    let estadoTag = '';
    if(n.estado === 'alerta'){
    ...
    // Atajo a la configuración de la NAP. Nació de un problema concreto: varias
    // están mal georreferenciadas y, viéndolas en el mapa, corregirlas obligaba
    // a irse a la tabla de NAPs y buscarlas por nombre.
    const editarBtn = puedeEditar('naps')
      ? `<button class="btn btn-gray btn-sm" style="margin-top:.3rem;width:100%"
                 onclick="editarNapDesdeMapa(${n.id})"
                 title="Abrir la configuración para corregir coordenadas u otros datos">📍 Editar ubicación / datos</button>`
      : '';
    // En modo reubicar el marcador se puede arrastrar. Fuera de ese modo NO:
    // un arrastre accidental movería una NAP sin que nadie se entere.
    const arrastrable = _modoReubicarNap && puedeEditar('naps');
    const marker=L.marker([n.lat,n.lng],{icon, draggable: arrastrable}).bindPopup(`
      ${estadoTag}<b>${escHtml(n.nombre)}</b><br>
      Ocupación: ${escHtml(n.total)}/${escHtml(n.capacidad)} <br>
      <div style="margin:.3rem 0">${dotsHtml}</div>
      ${(n.estado==='alerta'||n.estado==='mantenimiento')?verBtn:''}
      ${editarBtn}
    `,{maxWidth:300});
    if(arrastrable){
      marker.on('dragend', ev => _confirmarReubicacion(n, ev.target, [n.lat, n.lng]));
    }
    markers.push(marker);
  });
...
/* ════════════════════════════════════════════════════════════════
   Corrección de coordenadas de NAPs

   Dos caminos, porque son dos situaciones distintas:

   1) **Modo reubicar** (chip del mapa): los marcadores se vuelven
      arrastrables. Es lo más rápido cuando hay varias mal puestas y
      se ve en pantalla dónde tendrían que estar.
   2) **Elegir en el mapa** (botón del modal): se está editando una NAP
      y falta la coordenada. Manda a hacer un clic y vuelve al modal.

   Los dos terminan en PUT /api/v2/naps/<id>/ubicacion, que toca sólo
   lat y lng — el PUT heredado reescribe la ficha entera.
   ════════════════════════════════════════════════════════════════ */

let _modoReubicarNap = false;

function toggleReubicarNaps(){
  if(!puedeEditar('naps')){ alert('⛔ No tenés permisos para editar NAPs'); return; }
  _modoReubicarNap = !_modoReubicarNap;

  // Sin la capa de NAPs encendida el modo no se ve: se prende sola.
  if(_modoReubicarNap && !mapaChips.naps){
    mapaChips.naps = true;
    document.getElementById('chip-naps')?.classList.add('active');
  }
  document.getElementById('chip-reubicar')?.classList.toggle('active', _modoReubicarNap);
  _bannerReubicar(_modoReubicarNap);
  reloadMapaNaps();
}

function _bannerReubicar(mostrar){
  let b = document.getElementById('nap-reubicar-aviso');
  if(!mostrar){ b?.remove(); return; }
  if(b) return;
  b = document.createElement('div');
  b.id = 'nap-reubicar-aviso';
  b.className = 'nap-modo-aviso';
  b.innerHTML = `📍 <b>Modo reubicar</b> — arrastrá una NAP hasta su posición real.
    Se te va a pedir confirmación antes de guardar.
    <button class="btn btn-gray btn-xs" style="margin-left:.6rem" onclick="toggleReubicarNaps()">Salir</button>`;
  document.body.appendChild(b);
}

/* Confirma y guarda un arrastre. Si se cancela o falla, el marcador vuelve. */
async function _confirmarReubicacion(nap, marker, origen){
  const destino = marker.getLatLng();
  const volver = () => marker.setLatLng(origen);

  const metros = _metrosEntre(origen[0], origen[1], destino.lat, destino.lng);
  if(metros < 1){ volver(); return; }          // temblor del mouse, no un movimiento

  const ok = await confirmar(
    `Mover "${nap.nombre}" ${metros >= 1000 ? (metros/1000).toFixed(1)+' km' : metros+' m'}?\n\n` +
    `Desde:  ${origen[0].toFixed(6)}, ${origen[1].toFixed(6)}\n` +
    `Hasta:  ${destino.lat.toFixed(6)}, ${destino.lng.toFixed(6)}`,
    {titulo:'Corregir ubicación de la NAP', ok:'Guardar posición', peligro:false, icono:'📍'});
  if(!ok){ volver(); return; }

  const r = await api(`/api/v2/naps/${nap.id}/ubicacion`, 'PUT',
                      {lat: destino.lat, lng: destino.lng});
  if(!r || r.error){
    volver();
    alert('❌ ' + ((r && r.error) || 'No se pudo guardar la ubicación'));
    return;
  }
  if(r.fuera_de_zona){
    alert('⚠️ Guardado, pero la coordenada cae fuera de la zona de cobertura. Revisala.');
  } else {
    toast(`"${nap.nombre}" reubicada (${r.metros_movidos} m)`, 'ok');
  }
  // Refrescar la capa: el popup guarda las coordenadas viejas en su closure.
  await reloadMapaNaps();
}

/* Misma fórmula que el backend, para que el número que se confirma sea el que
   después queda escrito en la auditoría. */
function _metrosEntre(lat1, lng1, lat2, lng2){
  const rad = Math.PI/180;
  const latMedia = (lat1 + lat2)/2 * rad;
  const dx = (lng2 - lng1) * rad * Math.cos(latMedia);
  const dy = (lat2 - lat1) * rad;
  return Math.round(Math.hypot(dx, dy) * 6371000);
}

/* Selector de coordenada desde el modal: esconde el modal, espera un clic en el
   mapa y vuelve con lat/lng cargadas. No cierra el modal —lo oculta— para no
   perder lo que ya se hubiera tipeado en los otros campos. */
async function elegirUbicacionNapEnMapa(){
  const modal = document.getElementById('modal-nap');
  const enMapa = document.getElementById('page-mapa')?.classList.contains('active');
  if(!enMapa){
    if(typeof navGo === 'function') navGo('mapa');
    // initMapa es asincrónico en la práctica (crea el contenedor de Leaflet)
    await new Promise(r => setTimeout(r, 350));
  }
  if(!map){ alert('El mapa todavía no está listo. Probá de nuevo en un segundo.'); return; }

  modal.style.display = 'none';

  const aviso = document.createElement('div');
  aviso.className = 'nap-modo-aviso';
  aviso.innerHTML = `📍 <b>Hacé clic en la ubicación correcta de la NAP</b>
    <span style="opacity:.8">· Esc para cancelar</span>`;
  ...
  const alHacerClic = e => {
    document.getElementById('mnap-lat').value = e.latlng.lat.toFixed(6);
    document.getElementById('mnap-lng').value = e.latlng.lng.toFixed(6);
    limpiar();
    toast('Coordenadas cargadas. Acordate de guardar.', 'ok');
  };
  const alPresionar = ev => { if(ev.key === 'Escape') limpiar(); };
...
/* Abre la configuración de una NAP desde su globo en el mapa.

   Se recuerda que vino del mapa para poder refrescar la capa al guardar: si no,
   el marcador se queda en la posición vieja hasta recargar la página, que es
   justamente lo que uno quiere ver corregido. */
let _napEditadoDesdeMapa = false;

async function editarNapDesdeMapa(napId){
  if(!puedeEditar('naps')){ alert('⛔ No tenés permisos para editar NAPs'); return; }
  if(map && map.closePopup) map.closePopup();
  await openModalNap(napId);
  // Se marca DESPUÉS de abrir, porque openModalNap lo resetea: si se abriera
  // otra NAP desde la tabla, no debería arrastrar este estado.
  _napEditadoDesdeMapa = true;
  // El foco va a la latitud: es el motivo por el que se abre desde el mapa.
  document.getElementById('mnap-lat')?.focus();
}
  ...
  let html = `<div class="modal-overlay" id="modal-afectados-tmp" style="display:flex" onclick="if(event.target===this)this.remove()">
    <div class="modal" style="max-width:560px">
      <div class="modal-hd" style="background:${r.nap_estado==='alerta'?'#dc3545':'#ff9800'};color:#fff">
        <span>${r.nap_estado==='alerta'?'⚠️ Clientes afectados':'🔧 Clientes en NAP'}: ${escHtml(r.nap_nombre)}</span>
      ...
      <div class="modal-bd">
        ...
        ${r.clientes.map(c=>{
          const nro = c.nro_cliente ? `<span class="badge b-fibra" style="font-family:monospace;margin-right:.3rem">#${escHtml(c.nro_cliente)}</span>` : '';
          return `<div style="padding:.5rem;border-bottom:1px solid var(--brd);cursor:pointer" onclick="document.getElementById('modal-afectados-tmp').remove();openModalCliente(${c.id})">
            <div>${nro}<b>${escHtml(c.nombre)}</b></div>
            <div style="font-size:.78rem;color:#666">${escHtml(c.tipo_servicio)||'—'} · ${escHtml(c.telefono)||'—'} · ${escHtml(c.direccion)||'—'}</div>
          </div>`;
...
function renderNapsTable(){
  ...
  tbody.innerHTML=filtered.map(n=>{
    ...
    const badgeClass = est==='alerta'?'b-rescision':est==='mantenimiento'?'b-pendiente':est==='inactivo'?'b-baja':est==='revision'?'b-rescision':'b-activo';
    const badgeIcon = est==='alerta'?'⚠️ ':est==='mantenimiento'?'🔧 ':est==='inactivo'?'⊘ ':est==='revision'?'🚩 ':'✓ ';
    ...
    const sobrecarga = n.sobrecargado
      ? `<span class="badge b-rescision" title="Supera el límite de 8 clientes" style="margin-left:.3rem">🚩 +${n.exceso} sobrecarga</span>`
      : '';
    const oltBadge = n.olt_nombre
      ? `<span class="badge b-fibra">${escHtml(n.olt_nombre)} PON${escHtml(n.olt_puerto||'?')}</span>
    ...
    const nombreCell = `<b style="cursor:pointer;color:var(--az)" onclick="showNapDetail(${JSON.stringify(n.nombre||'').replace(/"/g,'&quot;')})">🔗 ${escHtml(n.nombre)}</b>`;
    const rowStyle = est==='alerta'?'style="background:var(--card)"':est==='mantenimiento'?'style="background:var(--tint-ambar)"':est==='revision'?'style="background:var(--tint-rojo)"':'';
    return `<tr ${rowStyle}>
      <td>${nombreCell}</td>
      <td>${escHtml(n.localidad)||'—'}</td>
      <td>${cap}</td>
      <td>
        <div style="display:flex;align-items:center;gap:.4rem">
          <div style="width:60px;height:7px;background:var(--surf2);border-radius:4px;overflow:hidden">
            <div style="width:${Math.min(pct,100)}%;height:100%;background:${occColor}"></div>
          </div>
          <span style="font-size:.78rem">${n.total||0}/${cap}</span>${sobrecarga}
        </div>
      </td>
...
async function openModalNap(id=null){
  ...
  // abrirModal y no style.display: se abre desde el globo del mapa y puede
  // haber otro modal encima (por ejemplo el de afectados).
  abrirModal('modal-nap');
}
...
async function saveNap(_force){
  ...
  if(!id && (!red||!cdo||!napNum)){
    if(!await confirmar('Los campos Red, CDO o NAP no están completos. ¿Guardar con nombre manual?')) return;
  }
  ...
  if(r?.ok){
    ...
    loadNaps();
    loadNapDatalist();
    // Si se editó desde el mapa, el marcador tiene que saltar a la posición
    // nueva ahora: corregir una coordenada y no verlo moverse no sirve.
    if(_napEditadoDesdeMapa){
      _napEditadoDesdeMapa = false;
      if(typeof reloadMapaNaps === 'function') await reloadMapaNaps();
    }
    return;
  }
  if(r?.error === 'duplicado'){
    ...
    document.getElementById('napdup-mensaje').innerHTML =
      `Ya existe un NAP con el mismo <b>${escHtml(campoLbl)}</b>: <code style="background:var(--tint-ambar);padding:.1rem .35rem;border-radius:3px">${escHtml(r.valor)}</code>`;
...
async function forzarGuardadoNap(){
  if(!_napDuplicadoActual) return;
  if(!await confirmar(`¿Confirmás forzar el guardado a pesar del duplicado con "${_napDuplicadoActual.conflicto_nombre}"?\n\nEsta acción quedará registrada.`)) return;
  closeModal('modal-nap-duplicado');
  await saveNap(true);
}
...
async function deleteNap(id){
  ...
  if(!await confirmar('¿Eliminar esta NAP?')) return;
...
async function buscarNapActivacion(){
  ...
  drop.innerHTML=filtered.map(n=>`<div onclick="selectNapActivacion('${escJs(n.nombre)}',${n.libre||0})" style="padding:.4rem .7rem;cursor:pointer;border-bottom:1px solid var(--brd);font-size:.82rem">
    <b>${escHtml(n.nombre)}</b> <span style="color:${(n.libre||0)>0?'var(--vd)':'var(--rj)'}">libre: ${n.libre||0}</span>
  </div>`).join('');
...
// ── Censo de NAP: técnico, asistente, fecha, bocas físicas ──
async function abrirCensoNap(nid, nombre, registrados){
  const tecnico = await pedirDato(`Censar NAP "${nombre}"\n\n¿Qué técnico la censó?`, '', {titulo:'Censo de NAP'});
  if(!tecnico) return;
  // Asistente: eaguiar o pperette
  const asistente = await pedirDato(`¿Quién asistió el censo?\n\nEscribí: eaguiar o pperette`, 'eaguiar', {titulo:'Asistente'});
  if(asistente === null) return;
  const bocasStr = await pedirDato(`¿Cuántas bocas OCUPADAS contaste físicamente en la NAP?\n\n(En el sistema figuran ${registrados} clientes)`, String(registrados), {titulo:'Bocas físicas', tipo:'number'});
  if(bocasStr === null) return;
  const bocas = parseInt(bocasStr);
  const notas = await pedirDato('Notas del censo (opcional):', '', {titulo:'Notas'}) || '';
  const r = await api(`/api/naps/${nid}/censar`, 'POST', {
    tecnico, asistente, bocas_fisicas: isNaN(bocas)?null:bocas, notas
  });
  if(r && r.ok){
    if(r.discrepancia){
      alert(`✅ Censo registrado.\n\n⚠ DISCREPANCIA detectada:\nBocas físicas: ${r.discrepancia.fisicas}\nRegistrados en sistema: ${r.discrepancia.registrados}\nDiferencia: ${r.discrepancia.diferencia > 0 ? '+' : ''}${r.discrepancia.diferencia}\n\nHay que reconciliar el registro de esta NAP.`);
    } else {
      toast('✅ Censo registrado correctamente', 'ok');
    }
    showNapDetail(nombre); // refrescar el modal
  } else {
    alert(r?.error || 'No se pudo registrar el censo');
  }
}

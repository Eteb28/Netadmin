...
async function loadIncidencias(){
  ...
  list.innerHTML=d.map(i=>`<div class="inc-card ${i.prioridad}" style="margin-bottom:.5rem">
    <div style="display:flex;align-items:flex-start;gap:.4rem;flex-wrap:wrap">
      <span class="badge b-${i.prioridad}">${escHtml(i.prioridad).toUpperCase()}</span>
      <b>${escHtml(i.titulo)}</b>
      <span class="badge b-${i.estado}" style="margin-left:auto">${escHtml(estadoServLabel(i.estado))}</span>
    </div>
    <div style="font-size:.78rem;color:var(--txt2);margin:.3rem 0">${escHtml(i.tipo.replace(/_/g,' '))} ${i.afectados?'| Afectados: '+escHtml(i.afectados):''} ${i.tecnico?'| Técnico: '+escHtml(i.tecnico):''}</div>
    ${i.descripcion?`<div style="font-size:.78rem;margin-bottom:.3rem">${escHtml(i.descripcion)}</div>`:''}
    ${i.resolucion?`<div style="font-size:.75rem;color:var(--vd);border-top:1px solid var(--brd);padding-top:.3rem;margin-top:.3rem">✅ ${escHtml(i.resolucion)}</div>`:''}
    <div style="font-size:.7rem;color:var(--txt2)">Inicio: ${escHtml(i.fecha_inicio?.slice(0,16)||'')} ${i.fecha_cierre?'| Cierre: '+escHtml(i.fecha_cierre.slice(0,16)):''}</div>
    <div style="margin-top:.4rem;display:flex;gap:.3rem;flex-wrap:wrap">
      ...
      ${i.estado!=='cerrada'?`<button class="btn btn-xs" style="background:#1565c0;color:#fff" onclick="notificarAfectados(${i.id})">📢 Afectados</button>`:''}
      ${i.estado!=='cerrada'?`<button class="btn btn-xs" style="background:#455a64;color:#fff" onclick="descargarAfectadosPDF(${i.id})">📄 PDF afectados</button>`:''}
      <button class="btn btn-rj btn-xs" onclick="deleteInc(${i.id})">🗑</button>
    </div>
    <div id="inc-afectados-panel"></div>
  </div>`).join('');
}
...
// ═══ NOTIFICACIÓN DE AFECTADOS (importado de v6.2) ═══
function descargarAfectadosPDF(incId){
  // Abre el PDF de clientes afectados en pestaña nueva (se descarga por Content-Disposition)
  window.open(`/api/incidencias/${incId}/afectados/pdf`, '_blank');
}

async function notificarAfectados(incId){
  const data = await api(`/api/incidencias/${incId}/afectados`);
  if(!data) return;
  const el = document.getElementById('inc-afectados-panel');
  if(!el) return;
  el.innerHTML = `
    <div style="background:var(--card);border-radius:10px;padding:.8rem;margin-top:.8rem;box-shadow:0 1px 4px rgba(0,0,0,.06)">
      <h3 style="margin:0 0 .5rem;font-size:.95rem">📢 Clientes afectados: ${data.total_afectados}</h3>
      <div style="background:var(--tint-azul);padding:.5rem;border-radius:6px;font-size:.82rem;margin-bottom:.5rem">
        <b>Mensaje sugerido:</b> ${data.mensaje_sugerido}
      </div>
      <div style="display:flex;gap:.3rem;margin-bottom:.5rem">
        <button class="btn btn-prim btn-xs" onclick="copiarTelefonos()">📋 Copiar teléfonos</button>
        <button class="btn btn-xs" style="background:#25D366;color:#fff" onclick="abrirWhatsApp()">💬 WhatsApp (1er cliente)</button>
        <button class="btn btn-gray btn-xs" onclick="exportAfectadosCSV(${incId})">📥 CSV</button>
      </div>
      <div id="telefonos-data" style="display:none">${data.telefonos.join('\\n')}</div>
      <table class="fin-table" style="font-size:.78rem">
        <thead><tr><th>Cliente</th><th>Teléfono</th><th>Localidad</th><th>Tipo</th></tr></thead>
        <tbody>${data.clientes.map(c=>`<tr>
          <td><b>${c.nombre}</b><br><small>${c.nro_cliente||''}</small></td>
...
function _normalizarTelArg(tel){
  // Normaliza un teléfono argentino al formato que espera wa.me: 549XXXXXXXXXX
  let t = (tel||'').replace(/[^0-9]/g, '');  // solo dígitos
  if(!t) return '';
  // Quitar 0 inicial (prefijo nacional) y 15 (celular viejo) si están
  t = t.replace(/^0/, '');
  // Si ya empieza con 54, dejarlo; si no, agregarlo
  if(!t.startsWith('54')) t = '54' + t;
  // Asegurar el 9 de celular después del 54 (si no está)
  if(t.startsWith('54') && !t.startsWith('549')) t = '549' + t.slice(2);
  return t;
}

function abrirWhatsApp(){
  const tels = (document.getElementById('telefonos-data')?.textContent || '').split('\n').filter(t=>t);
  if(!tels.length){ alert('No hay teléfonos'); return; }
  const tel = _normalizarTelArg(tels[0]);
  if(!tel || tel.length < 12){ alert('El primer teléfono no tiene un formato válido'); return; }
  const msg = 'Estimado cliente, le informamos que estamos trabajando en la resolución de una incidencia en su zona. Disculpe las molestias.';
  window.open(`https://wa.me/${tel}?text=${encodeURIComponent(msg)}`, '_blank');
}

function copiarTelefonos(){
  const tels = document.getElementById('telefonos-data')?.textContent || '';
  navigator.clipboard.writeText(tels).then(()=>alert('✅ Teléfonos copiados al portapapeles'));
}

async function exportAfectadosCSV(incId){
  const data = await api(`/api/incidencias/${incId}/afectados`);
  if(!data) return;
  let csv = 'Nombre,Nro Cliente,Telefono,Email,Localidad,Plan,Tipo\n';
  data.clientes.forEach(c=>{
    csv += `"${c.nombre}","${c.nro_cliente||''}","${c.telefono||''}","${c.email||''}","${c.localidad||''}","${c.plan||''}","${c.tipo_servicio||''}"\n`;
  });
  const blob = new Blob([csv], {type:'text/csv'});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = `afectados_incidencia_${incId}.csv`;
  a.click();
}

      ...
      <table class="tbl" style="font-size:.85rem">
        ...
        ${_permUsuarios.map(u => {
          ...
          if(p.modulos === '*' || u.permisos === 'root' || u.permisos === '*'){
            resumen = '<span style="color:#4FB88A;font-weight:600">✓ Acceso total</span>';
          } else if(Array.isArray(p.modulos)){
            resumen = `<span title="${escAttr(p.modulos.join(', '))}">${p.modulos.length} módulos · ${escHtml((p.acciones||[]).join(', '))||'sin acciones'}</span>`;
          } else {
          ...
          return `<tr>
            ...
            <td><span class="badge ${u.rol==='admin'?'b-fibra':u.rol==='tecnico'?'b-pendiente':'b-inalambrico'}">${escHtml(u.rol)}</span></td>
...
function _renderPermisosMatriz(){
  ...
  // Header
  let html = '<thead><tr><th style="text-align:left;padding:.4rem">Módulo</th>';
  acciones.forEach(a => {
    html += `<th style="padding:.3rem;font-size:.75rem;width:60px">${a.label}</th>`;
  });
  html += '</tr></thead><tbody>';

  // Agrupar módulos por su campo 'grupo' (si existe), preservando el orden del catálogo
  const grupos = [];
  const idxGrupo = {};
  _permCatalogo.modulos.forEach(m => {
    const g = m.grupo || 'Otros';
    if(!(g in idxGrupo)){ idxGrupo[g] = grupos.length; grupos.push({nombre:g, mods:[]}); }
    grupos[idxGrupo[g]].mods.push(m);
  });

  const nCols = acciones.length + 1;
  grupos.forEach(grupo => {
    // Fila cabecera de grupo con checkbox "marcar todo el grupo"
    html += `<tr class="perm-grupo-row">
      <td colspan="${nCols}" style="padding:.4rem .3rem;font-weight:600;font-size:.78rem;letter-spacing:.03em;opacity:.85">
        <label style="display:flex;align-items:center;gap:.4rem;cursor:pointer">
          <input type="checkbox" onchange="onGrupoToggle(this,'${grupo.nombre}')" ${wildcard?'disabled':''}>
          ${grupo.nombre}
        </label>
      </td></tr>`;
    grupo.mods.forEach(m => {
      const moduloEnabled = _moduloHabilitado(m.key);
      const moduloChecked = wildcard || moduloEnabled;
      html += `<tr data-grupo="${grupo.nombre}" ${moduloChecked?'':'style="opacity:.55"'}>`;
      html += `<td style="padding:.3rem .3rem .3rem 1.2rem">
...
function onModuloToggle(cb){
  ...
  // Re-render para habilitar/deshabilitar las acciones de esa fila
  _renderPermisosMatriz();
}

// Marcar/desmarcar todos los módulos de un grupo de una vez
function onGrupoToggle(cb, grupoNombre){
  if(!Array.isArray(_permActual.modulos)) _permActual.modulos = [];
  const mods = (_permCatalogo.modulos || []).filter(m => (m.grupo||'Otros') === grupoNombre);
  mods.forEach(m => {
    if(cb.checked){
      if(!_permActual.modulos.includes(m.key)) _permActual.modulos.push(m.key);
    } else {
      _permActual.modulos = _permActual.modulos.filter(k => k !== m.key);
    }
  });
  _renderPermisosMatriz();
}

  ...
  if (last && last.resumen) {
    try {
      ...
      for (const [mod, m] of Object.entries(resumen)) {
        if (m.error) {
          items.push(`<div class="sync-mod-line err">✗ <b>${escHtml(mod)}</b>: ${escHtml(m.error)}</div>`);
        } else {
          const detalle = Object.entries(m)
            .filter(([k]) => !['modulo','duracion_seg','detalle_errores'].includes(k))
            .map(([k,v]) => `${escHtml(k)}=${escHtml(v)}`).join(', ');
          items.push(`<div class="sync-mod-line">✓ <b>${escHtml(mod)}</b> (${m.duracion_seg||0}s): ${detalle}</div>`);
        }
      }
    ...
    <div class="sync-row">
      <div class="sync-card">
        ...
        ${last ? `
          ...
          <div class="sync-line muted">Iniciado por: ${escHtml(last.iniciado_por) || '?'}</div>
          ${last.errores ? `<div class="sync-errores"><b>Errores:</b><pre>${escHtml(last.errores)}</pre></div>` : ''}
...
async function testPG() {
  ...
  try {
    ...
    if (data.ok) {
      ...
      el.innerHTML = `<span style="color:#dc2626">✗ Error: ${escHtml(data.error) || 'desconocido'}</span>`;
    }
  } catch (e) {

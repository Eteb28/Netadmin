...
class TransporteInteractivo(TransporteCLIBase):
    ...
    def _requiere_login_interactivo(self) -> bool:
        """¿Hay que tipear usuario y contraseña en la sesión?

        Telnet sí; SSH ya autenticó en el handshake.
        """
        return True
    ...
    def _leer_hasta(self, patrones: tuple[re.Pattern[bytes], ...]) -> bytes:
        """Lee hasta encontrar alguno de los patrones, o hasta el timeout.

        Contesta la paginación sola: si el equipo ignoró ``terminal length 0``,
        un ``--More--`` sin responder deja la sesión colgada para siempre
        (riesgo R3).
        """
        ...
        while True:
            ...
            if time.monotonic() > limite:
                raise ErrorTiempoAgotado(
                    f"{self.host} no completó la respuesta en {self.timeout:g} s. "
                    f"Último fragmento: {acumulado[-120:]!r}"
                )
    ...
    def _autenticar(self) -> None:
        ...
        if self._requiere_login_interactivo():
            ...
            salida = self._leer_hasta((PROMPT_COMANDO,))

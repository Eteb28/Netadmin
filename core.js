...
function estadoLabel(e){
  return {activo:'Activo',suspendido:'Suspendido',rescision:'Pte.Rescisión',baja:'Baja'}[e]||e||'—';
}
      ...
      <div style="border:1px solid var(--brd);border-top:none;border-radius:0 0 7px 7px">
        ${items.map(s=>`<div style="padding:.5rem .7rem;border-bottom:1px solid var(--brd);display:flex;align-items:flex-start;gap:.5rem;flex-wrap:wrap">
          ...
          <div style="flex:1;min-width:150px">
            ...
            <div style="font-size:.77rem;color:var(--txt2)">${s.cliente_nombre||'Sin cliente'} ${s.cliente_tel?'| 📞 '+s.cliente_tel:''}</div>
            <div style="font-size:.73rem;color:var(--txt2)">📍 ${s.cliente_dir||'—'}, ${s.cliente_localidad||''}</div>
          ...
          <div style="display:flex;flex-direction:column;gap:.2rem;align-items:flex-end">
            ${s.tecnico?`<span class="badge b-normal">👷 ${s.tecnico}</span>`:'<span style="font-size:.72rem;color:var(--rj)">Sin técnico</span>'}
            <span class="badge b-${s.estado}">${estadoServLabel(s.estado)}</span>
            <button class="btn btn-gray btn-xs" onclick="openProgramarSvc(${s.id},'${(s.tecnico||'').replace(/'/g,"\\'")}','${s.fecha_programada||''}')">📅 Reprogramar</button>
          </div>
        </div>`).join('')}
      </div>
    </div>`;
